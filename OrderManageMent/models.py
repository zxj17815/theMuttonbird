...
class Order(models.Model):
    ...
    state=models.IntegerField("State",choices=((0,'待付款'),(1, '待发货'),(2, '待确认'),(3, '退货中'),(4,'退款中'),(5,'已完成')),default=0,help_text='int，订单状态[0:待付款,1:待发货,2:待确认,3:退货中,4:退款中,5:已完成]')
...
class Refund(models.Model):
    ...
    images=models.ImageField("Image", blank=True, null=True, upload_to=user_directory_path,height_field=None, width_field=None, max_length=None, help_text='退货附图')
    price=models.FloatField("Price",help_text='待退款金额')
    refund_type=models.IntegerField("RefundType",choices=((0,'仅退款'),(1,'退货退款')),default=0,help_text='int，退回类型[0:仅退款,1:退货退款]')
    state=models.IntegerField("State",choices=((0,'待通过'),(1, '待退货'),(2, '待确认'),(3, '已完成'),(4, '未通过')),default=0,help_text='int，状态[0:待通过,1:待退货,2:待确认,3:已完成,4:未通过]')
    re_extra=models.TextField("ReExtra",help_text='商家答复信息',null=True, blank=True)
    customer_express=models.TextField("CustomerExpress",null=True, blank=True,help_text='text，客户寄出的快递单号')
